...
function enableButtons () {
    ...
    // Update the button label now that the button is active
    btn.value="Click me for a greeting with period";

    // Set the onclick action for the third button
    btn = document.getElementById("input_greet_by_name_and_period_advanced");
    btn.onclick=function(){greetByPeriodAdvanced();};

    // Update the button label now that the button is active
    btn.value="Click for a advanced greeting with period";
}
...
function greetByPeriodAdvanced(){
    var name = document.getElementById("name_field").value;
    var currentDate = new Date();
    var period;
    currentDate.getHours()> 12 ? period = "Evening": period ="Morning";
    var request = gapi.client.helloworldendpoints.greetByPeriod({'name': name, 'period':period});
    request.execute(sayHelloCallback);
}
